...
import { ShoppingCart, Plus, Minus, Trash2, CreditCard, Mail, Calendar } from "lucide-react"
import {
  Sheet,
  SheetContent,
  SheetDescription,
...
export const EventsDashboard: FC<EventsDashboardProps> = ({ initialEvents }) => {
  ...
  const removeFromBasket = (eventId: number) => {
    const event = events.find(e => e.id === eventId)
    const existingItem = basket.find(item => item.eventId === eventId)
    if (existingItem && event) {
  ...
  const createCalendarInvite = (ticket: Ticket) => {
    const event = events.find(e => e.id === ticket.eventId)
    if (event) {
      const calendarEvent = {
        title: event.name,
  ...
  const checkout = () => {
    const newTickets: Ticket[] = basket.flatMap(item =>
      ...
      <div className="container mx-auto p-4">
          ...
          {loading ? (
              ...
              events.map((event) => (
                  <Card key={event.id} className="cursor-pointer" onClick={() => setSelectedEvent(event)}>
                    ...
                    <CardContent>
                      <Image src={event.filePath} alt={event.name} className="w-full h-40 object-cover mb-4 rounded" width={100} height={100}/>
        ...
        <Dialog open={!!selectedEvent} onOpenChange={() => setSelectedEvent(null)}>
          <DialogContent>
            ...
            <div className="mt-4">
              <Image src={selectedEvent?.filePath ?? ""} alt={selectedEvent?.name ?? "Image"} width={100} height={100} className="w-full h-48 object-cover rounded mb-4" />
